...
void AppWindow::handle ( const Event& e )
{
    ...
    if ( e.type == MouseDown )
    {
        ...
        for(selected = 0; selected < windows.count(); selected++)
        {
            if(windows.fetch(selected).contains(pos))
                break;
        }

        if(selected < windows.count())
        {
            windows.promote(selected);
            selected = 0;

            dragOffset = windows.fetch(selected).getOrigin() - pos ;
        }
    }
    ...
    if ( e.type == Motion)
    {
        if(selected < windows.count())
        {
            ...
            windows.fetch(selected).setOrigin(pos);
        }
    }
...
void AppWindow::draw ()
{
    ...
    for(RectStash::iterator i = 0; i < windows.count(); i++)
    {
        Rect& r = windows.fetch(i);        

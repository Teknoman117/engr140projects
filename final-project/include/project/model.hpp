...
private:
    ...
    inline T* GetProgram()
    {
        return static_cast<T*>(programWrapper);
    }
...
public:
    ...
    Model(const Json::Value& root, const std::string& directory, Resources& resources);
    ...
    // Render the model
    void Draw(const std::string& program, const glm::mat4& offset);
    void Draw(const std::string& program, const Node& _skeleton, const glm::mat4& offset);
    ...
    static Model* LoadFromJsonFile(const std::string& directory, const std::string& name, const Transform& offset, Resources& resources);
    static Model* LoadFromCompressedJsonFile(const std::string& directory, const std::string& name, const Transform& offset, Resources& resources);
};

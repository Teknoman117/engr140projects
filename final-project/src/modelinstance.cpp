...
#include <project/animationtransition.hpp>
 ...
 */
void ModelInstance::Draw(const std::string& program)
{
    // Push the model transform onto the program's matrix stack
    node->Recalculate();
    animation->Skeleton()->Recalculate();
    model->Draw(program, *(animation->Skeleton()), node->TransformMatrix());
}
 ...
 *
 */
glm::mat4 ModelInstance::GetTransformOfSkeletalNode(std::string name)
{
 ...
 */
ModelInstance* ModelInstance::LoadManifestEntry(const Json::Value& model, Resources& resources)
{
    ...
    if(compressed)
    {
        instance = new ModelInstance(Model::LoadFromCompressedJsonFile(directory, path, transform, resources));
    } else
    {
        instance = new ModelInstance(Model::LoadFromJsonFile(directory, path, transform, resources));
    }
...
// TEST TEST TEST
bool ModelInstance::PlayAnimation(const std::string name, double now)
{
